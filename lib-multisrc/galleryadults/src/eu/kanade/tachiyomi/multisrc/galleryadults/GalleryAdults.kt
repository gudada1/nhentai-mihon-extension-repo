...
abstract class GalleryAdults(
    ...
    override fun searchMangaParse(response: Response): MangasPage {
        ...
        if (loginRequired(document, response.request.url.toString())) {
            throw Exception("请先通过 WebView 登录后再查看收藏")
        } else {
    ...
    override fun getFilterList(): FilterList {
        ...
        if (supportAdvancedSearch) {
            filters.addAll(
                listOf(
                    Filter.Separator(),
                    Filter.Header("高级筛选会忽略普通搜索词。多个条件用英文逗号 (,) 分隔，前面加减号 (-) 表示排除。"),
                    TagsFilter(),
                    ParodiesFilter(),
                    ArtistsFilter(),
                    CharactersFilter(),
    ...
    protected open fun getCategoryURIs() = listOf(
        SearchFlagFilter("漫画", "m"),
        SearchFlagFilter("同人志", "d"),
        SearchFlagFilter("欧美", "w"),
        SearchFlagFilter("图集", "i"),
        SearchFlagFilter("画师 CG", "a"),
        SearchFlagFilter("游戏 CG", "g"),
    )
