...
import eu.kanade.tachiyomi.extension.all.hdoujincn.Entries.Entry
...
class HDoujin(
    ...
    private val clearanceClient = network.client.newBuilder()
        .addInterceptor { chain ->
            ...
            val clearance = getClearance()
                ?: throw IOException("请打开 WebView 刷新访问令牌")
    ...
    override fun setupPreferenceScreen(screen: PreferenceScreen) {
        ListPreference(screen.context).apply {
            key = PREF_IMAGE_RES
            title = "图片分辨率"
            entries = arrayOf("780x", "980x", "1280x", "1600x", "原图")
        ...
        EditTextPreference(screen.context).apply {
            ...
            title = "浏览/搜索时始终包含的标签"
            summary = "多个标签用英文逗号 (,) 分隔。\n" +
                "当前：${alwaysIncludeTags()}"
        }.also(screen::addPreference)
        EditTextPreference(screen.context).apply {
            key = PREF_EXCLUDE_TAGS
            title = "浏览/搜索时始终排除的标签"
            summary = "多个标签用英文逗号 (,) 分隔。支持标签类型，例如 female:hairy、male:hairy；未指定时默认使用 tag。\n" +
                "示例：ai generated, female:hairy, male:hairy\n" +
                "当前：${alwaysExcludeTags()}"
        }.also(screen::addPreference)
    }
    companion object {
